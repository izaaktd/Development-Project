...
Developer:  Izaak Dittrich & Viviance Nguyen
Date:       April 21, 2022
...
class App():
    ...
    def create_app(self):
        ...
        def screen2():
            ...
            Label(frame, text="Time In (Hour)", bg='blue', fg='white', font='times 16 bold').grid(row=3, column=1,pady = 15,padx =20,sticky=W)                                                                                        
            Label(frame, text="Time In (Minute)", bg='blue', fg='white', font='times 16 bold').grid(row=4, column=1,pady = 15,padx =20,sticky=W)                                                                                          
            Label(frame, text="Time Out (Hour)", bg='blue', fg='white', font='times 16 bold').grid(row=5, column=1,pady = 15, padx =20,sticky=W)                                                                                            
            Label(frame, text="Time Out (Minute)", bg='blue', fg='white', font='times 16 bold').grid(row=6, column=1,pady = 15, padx =20,sticky=W)                                                                                               
            Label(frame, text="Total Hours", bg='blue', fg='white', font='times 16 bold').grid(row=7, column=1,pady = 15,padx =20,sticky=W)                                                                                     
            Label(frame, text="Total Minute", bg='blue', fg='white', font='times 16 bold').grid(row=8, column=1,pady = 15,padx =20,sticky=W)


            time_inHVar = StringVar()
            Entry(frame, textvariable=time_inHVar, justify=RIGHT).grid(row=3, column=2, padx= 20)
            time_inMVar = StringVar()
            Entry(frame, textvariable=time_inMVar, justify=RIGHT).grid(row=4, column=2, padx=20)
            time_outHVar = StringVar()
            Entry(frame, textvariable=time_outHVar, justify=RIGHT).grid(row=5, column=2,padx =20)
            time_outMVar = StringVar()
            Entry(frame, textvariable=time_outMVar, justify=RIGHT).grid(row=6, column=2,padx =20)
            total_hoursVar = StringVar()
            Label(frame, textvariable=total_hoursVar,width=21).grid(row=7, column=2)
            total_minVar = StringVar()
            Label(frame, textvariable=total_minVar,width=21).grid(row=8, column=2)
            ...
            def computetotalhours():
                ...
                total_hoursVar.set(format(Total_hours))
                total_minVar.set(format(Total_min, ".1f"))
            ...
            def computewage():
                total_wage = getwage(
                    float(domestic_hoursVar.get()) * 43,
                    float(international_hoursVar.get()) * 56)

                total_wageVar.set(format(total_wage, "10.2f"))

            Button(frame, text="Compute Total Hours", command=computetotalhours,fg= 'black',bg='blue',highlightbackground='orange',font='times 12 bold').grid(row=9, column=2, padx =30)
